...
def get_forecast(page_num, lat, lng) -> pd.DataFrame:
    ...
    response = requests.get(FCST_URL, params=payload)
    print(response.content)
    data = response.json()
    forecast_data = data['response']['body']['items']['item']
...
def processing(**kwargs) -> pd.DataFrame:
    ...
    forecast_df = kwargs['ti'].xcom_pull(task_ids='get_forecast_task')
    
    forecast_df['fcst_datetime'] = pd.to_datetime(forecast_df['fcstDate'] + forecast_df['fcstTime'], format='%Y%m%d%H%M')
    latest_forecast_df = forecast_df.sort_values(by='fcst_datetime').drop_duplicates(subset=['category'], keep='last')
...
def save_file(**kwargs):
    ...
    latest_forecast_df = kwargs['ti'].xcom_pull(task_ids='processing_task')
...
with DAG(
    ...
    get_forecast_task = PythonOperator(
        ...
        op_kwargs={
            'page_num': 1,
            'lat': 37,  # Seoul's latitude
            'lng': 127  # Seoul's longitude
        }
    )
