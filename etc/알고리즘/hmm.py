...
class Model(object):
    ...
    def __init__(self, states, symbols, start_prob=None, trans_prob=None, emit_prob=None):
        ...
        self._trans_prob = _normalize_prob_two_dim(
            trans_prob, self._states, self._states)
        self._emit_prob = _normalize_prob_two_dim(
            emit_prob, self._states, self._symbols)
    ...
    def _forward(self, sequence):
        ...
        for state in self._states:
            alpha[0][state] = self.start_prob(
                state) * self.emit_prob(state, sequence[0])
    ...
    def decode(self, sequence):
        ...
        for index in range(1, sequence_length):
            ...
            for state_to in self._states:
                ...
                max_state = None
                for state_from in self._states:
                    prob = delta[state_from] * \
                        self.trans_prob(state_from, state_to)
                    if prob > max_prob:
                        max_prob = prob
                        max_state = state_from
                delta_bar[state_to] = max_prob * \
                    self.emit_prob(state_to, sequence[index])
        ...
        for state in self._states:
            ...
            if gamma_sum > 0:
                ...
                for state_to in self._states:
                    ...
                    self._trans_prob[state][state_to] = (
                        smoothing + xi_sum) / denominator
            else:
                for state_to in self._states:
